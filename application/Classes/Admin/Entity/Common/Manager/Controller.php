 ...
 *
 */
abstract class Admin_Entity_Common_Manager_Controller extends MVC_Controller_Default
{
	...
	protected null|Entity_Basic|Admin_Entity_Common_Interface $current_item = null;
	...
	protected function newItemFactory() : Entity_Basic|Admin_Entity_Common_Interface
	{
	...
	public function add_Action() : void
	{
		...
		$this->content->output(
			Admin_Managers::EntityEdit_Common()->renderAdd(
				item: $item,
	...
	public function edit_main_Action() : void
	{
		...
		$this->content->output(
			Admin_Managers::EntityEdit_Common()->renderEditMain(
				item: $item,
				tabs: $this->tabs,
	...
	public function edit_main_handleActivation() : void
	{
		...
		$logEntityActivation = function() {
			$entity_type = $this->current_item->getEntityType();
			$entity_id = $this->current_item->getId();
			
			Logger::success(
				event: 'entity_activated:'.$entity_type,
				event_message: 'Entity '.$entity_type.' \''.$this->current_item->getInternalName().'\' ('.$entity_id.') activated',
				context_object_id: $entity_id,
				context_object_name: $this->current_item->getInternalName()
			);
		};
		$logEntityDeactivation = function() {
			...
			Logger::success(
				...
				event_message: 'Entity '.$entity_type.' \''.$this->current_item->getInternalName().'\' ('.$entity_id.') deactivated',
				context_object_id: $entity_id,
				context_object_name: $this->current_item->getInternalName()
			);
		};
		...
		if($GET->exists('deactivate_entity')) {
			if($this->current_item->isActive()) {
				$this->current_item->deactivate();
				$logEntityDeactivation();
			}
			
			Admin_Managers::FulltextSearch()->updateIndex( $this->current_item );
