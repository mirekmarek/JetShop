...
class MagicTag_ProductName extends MagicTag
{
	public const ID = 'product_name';
	...
	public function generate( array $contexts ) : string
	{
		...
		return '%PRODUCT_NAME:'.$contexts['product_id'].'%';
	}
	...
	public function process( string $output ): string
	{
		...
		if(preg_match_all('/%PRODUCT_NAME:([0-9]+)%/', $output, $matches, PREG_SET_ORDER)) {
			foreach( $matches as $m ) {
				...
				$output = str_replace($orig_str, $name, $output);
			}
		}
