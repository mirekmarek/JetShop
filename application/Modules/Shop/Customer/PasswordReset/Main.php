 ...
 *
 */
class Main extends Application_Module implements Shop_Managers_CustomerPasswordReset, EMail_TemplateProvider, Shop_ModuleUsingTemplate_Interface
{
	...
	public function generateToken( Customer $user ) : string
	{
		...
		$email = $mail_template->createEmail( $user->getShop() );
		...
		return Shop_Pages::ResetPassword()->getURL(GET_params: ['validate'=>$user->getId(), 'key'=>$this->generateKey($user)]);
