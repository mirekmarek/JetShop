 ...
 *
 */
class Main extends Application_Module implements Shop_Managers_CustomerLogin, Shop_ModuleUsingTemplate_Interface
{
		...
		if(!static::$login_form) {
			...
			if($ajax) {
				static::$login_form->setAction(
					Shop_Pages::Login()->getURL(GET_params: ['action'=>'login_ajax'])
				);
			}
	...
	public static function getLogoutForm() : Form
	{
		if(!static::$logout_form) {
			...
			static::$logout_form->setAction(
				Shop_Pages::Login()->getURL(GET_params: ['action'=>'logout'])
			);
		}
	...
	{
		if(
			MVC::getPage()->getKey()==Shop_Pages::CustomerSection()->getKey() &&
