 ...
 * @author  
 */
namespace JetApplicationModule\Shop\ProductListing;
 ...
 *
 */
class Main extends Application_Module implements Shop_Managers_ProductListing, Shop_ModuleUsingTemplate_Interface
{
	...
	public function init( array $product_ids, int $category_id=0, string $category_name='', ?string $optional_URL_parameter = null ) : void
	{
		...
		if(is_array($numbers = $GET->getRaw('n'))) {
			...
			if($price_filter) {
				$this->listing->setNumbersFilter( $price_filter );
			}
