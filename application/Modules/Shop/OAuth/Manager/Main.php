 ...
 * @author  
 */
namespace JetApplicationModule\Shop\OAuth\Manager;
...
class Main extends Shop_Managers_OAuth implements Shop_ModuleUsingTemplate_Interface
{
	...
	public function getOAuthModules(): array
	{
		if($this->oauth_modules===null) {
			$this->oauth_modules = [];
			$modules = Managers::findManagers(Shop_OAuth_BackendModule::class, 'Shop.OAuth.Backend.');
			
			foreach( $modules as $oauth_module ) {
				$service_id = $oauth_module->getOAuthServiceID();
				
				$oauth_module->setHandlerUrl( Shop_Pages::OAuth()->getURL(path_fragments: [$service_id]) );
	...
	public function handle( Shop_OAuth_BackendModule $module ): void
	{
	...
	protected function handleRequest( Shop_OAuth_BackendModule $module ): void
	{
		...
		if(
			...
			($page = Shop_Pages::getPage( $redirect_page_id ))
		) {
	...
	protected function handleOAuthServiceReturn( Shop_OAuth_BackendModule $module ): void
	{
		$session = $this->getSession();
		...
		if(
			...
			if( $module->handleOAuthServiceReturn(
				...
				Http_Headers::movedTemporary(
					Shop_Pages::getPage( $redirect_page_id )->getURL( path_fragments: $path_fragments )
				);
			}
	...
	protected function authUserHandler(): Shop_OAuth_UserHandler
	{
		return new class extends Shop_OAuth_UserHandler {
			public function handle(): void
			{
				if( !($customer = Customer::getByOAuth( $this->getOauthService(), $this->getOauthUserId() )) ) {
					...
					if( !($customer = Customer::getByEmail( $this->getOauthUserEmail() )) ) {
						...
						$customer->setShop( Shops::getCurrent() );
	...
	public function renderLoginButton( Shop_OAuth_BackendModule $module ) : string
	{
		$view = $this->getView();
