...
class Controller_handleCategorySettings extends MVC_Controller_Default
{
	...
	{
		$this->category = $this->getContent()->getParameter('category');
		...
		$export_category_field = new Form_Field_Hidden('export_category', 'Heureka category ID:', $export_category_id);
		$export_category_field->setValidator(function( Form_Field_Hidden $field ) {
			...
			if(!isset($this->categories[$value])) {
				$field->setCustomError(Tr::_('Unknown Heureka category'));
				return false;
			}
