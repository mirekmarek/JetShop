...
class Listing_Filter_Status extends DataListing_Filter
{
	...
	{
		$this->status = Http_Request::GET()->getString('status', '', array_keys(WarehouseManagement_TransferBetweenWarehouses::getStatusScope()));
		if($this->status) {
	...
	{
		$options = [''=>Tr::_(' - all -')] + WarehouseManagement_TransferBetweenWarehouses::getStatusScope();
