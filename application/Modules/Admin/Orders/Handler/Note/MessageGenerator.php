...
abstract class Handler_Note_MessageGenerator {
	...
	{
		$view_dir = $view->getScriptsDir().'message/'.$order->getShopKey().'/'.$this->getKey().'/';
	...
	public function renderText( bool $append_signature) : string
	{
		...
		if($append_signature) {
			$text .= "\n\n".AdministratorSignatures::getSignature( $this->order->getShop() );
		}
	...
	public static function initGenerators( MVC_View $view, Order $order ) : void
	{
