...
class Handler_ChangeEMailAndPhone_Main extends Handler {
	...
	protected function init() : void
	{
		...
		if($this->order->getCustomerId()) {
			$email->setValidator(function() use ($email) {
				...
				if($new_email_address==$this->order->getEmail()) {
					return true;
				}
		...
		$phone->setDefaultValue( $this->order->getPhone() );
	...
	public function canBeHandled(): bool
	{
		if(
			!parent::canBeHandled() ||
			CustomerBlacklist::customerIsBlacklisted( $this->order->getEmail())
		) {
			return false;
		}
	...
	public function handle() : void
	{
		if($this->form->catchInput()) {
			...
			if($this->form->validate()) {
				...
				$change = $this->order->updateEmailAndPhone(
					new_email: $new_email,
					new_phone: $new_phone,
