...
class Handler_Note_Main extends Handler
{
	...
	protected Complaint_Note $new_note;
	protected ?Form $internal_note_form = null;
	protected ?Form $customer_message_form = null;
	
	protected function init() : void
	{
		$this->new_note = new Complaint_Note();
		$this->new_note->setComplaint( $this->complaint );
		...
		Handler_Note_MessageGenerator::initGenerators( $this->view, $this->complaint );
	}
	...
	{
		$dir = SysConf_Path::getData().'complaint_note_files_tmp/'.Auth::getCurrentUser()->getId().'/'.$this->complaint->getShop()->getKey().'/'.$this->complaint->getId().'/';
	...
	{
		$dir = SysConf_Path::getData().'complaint_note_files/'.Auth::getCurrentUser()->getId().'/'.$this->complaint->getShop()->getKey().'/'.$this->complaint->getId().'/';
	...
	public function getCustomerMessageForm() : Form
	{
		if(!$this->customer_message_form) {
			$new_note_text = new Form_Field_Textarea('new_note_text', '');
			$new_note_text->setDefaultValue(PHP_EOL.PHP_EOL.PHP_EOL.AdministratorSignatures::getSignature( $this->complaint->getShop() )  );
			$new_note_text->setFieldValueCatcher(function( $value ) {
				$this->new_note->setNote( $value );
			});
			...
			$email_subject->setDefaultValue( $template->createEmail( $this->complaint->getShop() )->getSubject() );
			...
			switch(Http_Request::GET()->getString('note-action')) {
				case 'add_note':
					...
					if($this->getCustomerMessageForm()->catch()) {
						...
						$this->new_note->setCustomerEmailAddress( $complaint->getEmail() );
						$this->saveFiles();
						$this->new_note->save();
						...
						$email = $template->createEmail( $this->complaint->getShop() );
					...
					AJAX::operationResponse(
						true,
						snippets: [
							'complaint-history' => $this->main_view->render('edit/history'),
	...
	/**
	 * @param Complaint_Note $note
	 * @return Handler_Note_File[]
	 */
	public function getNoteFiles( Complaint_Note $note ) : array
	{
		$files = [];
