...
abstract class Handler {
	...
	public function __construct( string $base_dir, MVC_View $main_view, MoneyRefund $money_refund )
	{
		...
		$this->view->setVar('money_refund', $money_refund);
	...
	public function canBeHandled() : bool
	{
		...
		if(
			$this->handleOnlyIfMoneyRefundIsEditable() &&
			!$this->money_refund->isEditable()
		) {
			return false;
		}
	...
	public static function initHandlers( MVC_View $main_view, MoneyRefund $money_refund ) : void
	{
		static::$handlers = [];
		...
		foreach($dirs as $path=>$dir) {
			 ...
			 * @var Handler $handler
			 */
			$handler = new $class_name( $path, $main_view, $money_refund );
