 ...
 * @author  
 */
namespace JetShopModule\Delivery\PersonalTakeover\CeskaPostaNaPostu;
...
{
	protected static string $method_code = 'PersonalTakeover.CeskaPostaNaPostu';
	...
	public function getCurrentPlaces( Shops_Shop $shop ): iterable
	{
		...
		$data = simplexml_load_file( 'http://napostu.ceskaposta.cz/vystupy/napostu.xml' );
		...
		foreach( $data as $item ) {
			...
			$place->setShopId( $shop->getId() );
			$place->setPlaceCode( (string)$item->PSC );
			$place->setMethodCode( static::$method_code );

			list($latitude, $longitude) = static::JTSK2WGS( (float)$item->SOUR_X, (float)$item->SOUR_Y );

			list( $street ) = explode(',', $item->ADRESA);
