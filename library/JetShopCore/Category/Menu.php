...
abstract class Core_Category_Menu
{
	...
	public static function generateAll() : void
	{
		foreach(Shops::getList() as $shop) {
			static::generate( $shop->getId() );
		}
	}

	public static function generate( string $shop_id ) : void
	{
		$tree = static::getTree( $shop_id );
		...
		foreach($tree as $node) {
			...
			$new_item = static::createMenuItem( $shop_id, $d );
	...
	protected static function createMenuItem( string $shop_id, array $data ) : Category_Menu_Item
	{
	...
	protected static function getTree( $shop_id ) : Data_Tree
	{
	 ...
	 */
	public static function getItems( int $parent_id, string $type='', string $shop_id='' ) : array
	{

		$all = static::getAllItems($shop_id);
	 ...
	 * @return Category_Menu_Item[]
	 */
	public static function getAllItems( string $shop_id='' ) : array
	{
		if(!$shop_id) {
			$shop_id = Shops::getCurrentId();
		}

		if(array_key_exists($shop_id, static::$menus )) {
			return static::$menus[$shop_id];
		}

		static::$menus[$shop_id] = Category_Menu_Item::fetch(
			where_per_model: [
				'shop_id' => $shop_id
			],
			order_by: 'priority',
		...
		foreach( static::$menus[$shop_id] as $item ) {
			$parent_id = $item->getParentCategoryId();
			if(
				$parent_id &&
				isset(static::$menus[$shop_id][$parent_id])
			) {
				static::$menus[$shop_id][$parent_id]->addChildren( $item );
			}
		}
