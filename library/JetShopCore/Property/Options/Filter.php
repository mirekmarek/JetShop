 ...
 *
 */
abstract class Core_Property_Options_Filter extends Property_Filter
{
	
	
	protected array|null $filtered_product_ids = null;
	
	public function __construct( ProductListing $listing, Property_Options $property )
	{
		...
		foreach( $property->getOptions() as $option ) {
			$option->initFilter( $listing, $property );
		}
	}
	...
	public function getAutoAppendProductFilterEditForm( Form $form ) : void
	{
		...
		foreach($this->property->getOptions() as $option) {
			...
			$field->setDefaultValue( $option->filter()->isSelected() );
			$form->addField($field);
	...
	public function initAutoAppendFilter( array $target_filter ) : void
	{
		...
		if(!empty($fd['options'])) {
			foreach($this->property->getOptions() as $option) {
				$option->filter()->setIsSelected( isset($fd['options'][$option->getId()]) );
			}
		}
	}
	...
	public function initByStateData( array $state_data ) : void
	{
		if($state_data['is_active']) {
			...
			foreach($this->property->getOptions() as $option) {
				$option->filter()->setIsSelected( in_array($option->getId(), $state_data['options']) );
			}
			
		} else {
			foreach($this->property->getOptions() as $option) {
				$option->filter()->setIsSelected( false );
			}
	...
	public function generateUrlPart() : string
	{
		...
		foreach($this->property->getOptions() as $o) {
			if($o->filter()->isSelected()) {
				$options[] = $o->getShopData($shop)->getUrlParam();
			}
		}
		if(!$options) {
			return '';
		...
		foreach($parts as $i=>$part) {
			if(stripos($part, $prefix)===0) {
				...
				foreach( $this->property->getOptions() as $option ) {
					...
					$url_param = $option->getShopData($shop)->getUrlParam();
					
					if(in_array($url_param, $options)) {
						$option->filter()->setIsSelected(true);
					}
				}
	...
	public function disableNonRelevantFilters() : void
	{
		$enabled_count = 0;
		foreach($this->property->getOptions() as $option) {
			if(!$option->filter()->getCount()) {
				$option->filter()->setIsDisabled(true);
			} else {
				$enabled_count++;
			}
		}
