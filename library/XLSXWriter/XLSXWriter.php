...
class XLSXWriter
{
	...
	private function initializeColumnTypes($header_types)
	{
		...
		foreach($header_types as $v)
		{
			...
			$column_types[] = array('number_format' => $number_format,//contains Excel formated like 'YYYY-MM-DD HH:MM:SS'
	...
	protected function styleFontIndexes()
	{
		...
		foreach($this->cell_styles as $i=>$cell_style_string)
		{
			...
			{
				if ( str_contains( $style['font-style'], 'bold' ) ) {$font['bold'] = true; }
				if ( str_contains( $style['font-style'], 'italic' ) ) {$font['italic'] = true; }
				if ( str_contains( $style['font-style'], 'strike' ) ) {$font['strike'] = true; }
				if ( str_contains( $style['font-style'], 'underline' ) ) {$font['underline'] = true; }
			}
