...
abstract class Core_Shops_Shop implements Form_Definition_Interface
{
	...
	{
		return $this->shop_code.'_'.$this->_locale;
	}
	...
	public function getDefaultPricelist() : Pricelists_Pricelist
	{
	...
	public function getDefaultAvailability(): Availabilities_Availability
	{
	...
	public function getTemplate() : Shop_Template
	{
		return new Shop_Template( $this->getTemplateRelativeDir() );
	}
	...
	protected function updateForm( Form $form ) : void
	{
		...
		$URLs_field->setValidator( function() use ($URLs_field) : bool {
			...
			foreach(Shops::getList() as $shop) {
				if($shop->getKey()==$this->getKey()) {
					continue;
				}
				foreach($shop->_URLs as $URL) {
					$known_URL[$URL] = $shop;
				}
			}
			...
			foreach($URLs as $URL) {
				...
				if(isset($known_URL[$URL])) {
					$URLs_field->setError('not_unique_URL', [
						'URL' => $URL,
						'SHOP' => $known_URL[$URL]->getShopName()
					]);
					return false;
				}
	...
	public function getEditForm() : Form
	{
		if( $this->_edit_form===null ) {
			...
			$form->field('shop_code')->setIsReadonly( true );
	...
	public function getCreateForm() : Form
	{
		if( $this->_add_form===null ) {
			...
			$locale_field->setValidator( function() use ($locale_field, $base_field) {
				...
				foreach(Shops::getList() as $shop) {
					if(
						$shop->getBaseId()==$selected_base_id &&
						$shop->getLocale()->toString()==$selected_locale
					) {
						$locale_field->setError('not_unique_locale', ['shop'=>$shop->getShopName()]);
						return false;
					}
				}
			...
			$pages_source_shop_filed = new Form_Field_Select('pages_source_shop', 'Copy pages from shop:');
			$pages_source_shop_filed->setSelectOptions( Shops::getScope() );
			$form->addField( $pages_source_shop_filed );
			
			
			
			$code_field = $form->field('shop_code');
	...
	public function catchCreateForm( string &$error_message ) : bool
	{
		...
		try {
			...
			$pages_source_shop = Shops::get( $form->field('pages_source_shop')->getValue() );
			
			$pages_source_dir = MVC::getBase( $pages_source_shop->getBaseId() )->getPagesDataPath( $pages_source_shop->getLocale() );
			$pages_target_dir = MVC::getBase( $this->_base_id )->getPagesDataPath( $this->_locale );
			
			IO_Dir::copy(
				source_path: $pages_source_dir,
				target_path: $pages_target_dir,
