...
#[DataModel_Definition(
	...
	database_table_name: 'number_series_counter_year',
)]
abstract class Core_NumberSeries_Counter_Year extends NumberSeries_Counter
{
	...
	public static function generate( EShopEntity_HasNumberSeries_Interface $entity, int $pad ) : string
	{
		...
		$where[] = 'AND';
		$where['year'] = $entity->getNumberSeriesEntityData()->format('Y');
		...
		foreach($current as $item ) {
			...
			return $entity->getNumberSeriesEntityData()->format('Y').str_pad( $count, $pad, '0', STR_PAD_LEFT );
		}
		...
		$item->year = $entity->getNumberSeriesEntityData()->format('Y');
		$item->count = $count;
		$item->save();
