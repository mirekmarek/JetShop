var Whisperer = function(
	...
	this.init = function() {
		...
		var orig_blur_handlers = jQuery._data(whisperer.input_field[0], "events")['blur'];

		if(orig_blur_handlers) {
			for( var c=0;c<orig_blur_handlers.length;c++ ) {
		...
		$(document).keydown(function(event){
			...
			var selected_row = whisperer.whisperer_items_area.find('.'+whisperer.selected_item_class);
			...
			switch( event.keyCode ) {
				...
				case KEY_ARROW_DOWN:
					if(selected_row.length) {
						var next_row = selected_row.next('.'+whisperer.item_class);
				...
				case KEY_ARROW_UP:
					if(selected_row.length) {
						var prev_row = selected_row.prev('.'+whisperer.item_class);
						if(prev_row.length) {
	...
	this.whisper = function() {
		...
		whisperer._whisperer_timeout_handler = setTimeout(function () {
			...
			var value = whisperer.input_field.val();

			if( value.length >= whisperer.min_str_len ) {
				var url = whisperer.data_request_url + encodeURIComponent(value);
	...
	this._onLoad = function( snippet ) {
		...
		var selected_row = whisperer.whisperer_items_area.find('.' + whisperer.selected_item_class);
		if (selected_row.length) {
			var row_height = selected_row.innerHeight();
			...
			var message = whisperer.whisperer_items_area.find('.' + whisperer.message_class);
			if (message.length) {
	...
	this._handleCancel_callOriginalBlurHandlers = function( e ) {
		for(var c=0; c<whisperer._original_blur_handlers.length; c++) {
	...
	this._handleItemFetch = function( row ) {
		...
		var row_top_offset = row.offset().top-whisperer.whisperer_items_area.offset().top;
		var ideal_row_position = row_height * whisperer.selected_row_positioning;
		var ideal_row_position_diff = row_top_offset-ideal_row_position;

		var scroll_to = whisperer.whisperer_items_area.scrollTop() + ideal_row_position_diff;
